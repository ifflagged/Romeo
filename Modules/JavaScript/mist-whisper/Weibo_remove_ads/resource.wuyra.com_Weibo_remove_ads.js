...
if (!$response || !$response.body) {
  ...
  try {
      ...
      if (url.includes("/2/cardlist")) {
        ...
        if (obj?.channelInfo?.channels?.length > 0) {
          ...
          for (let tab of obj.channelInfo.channels) {
            if (/selfrecomm/.test(tab?.flowId)) {
              // 关注页推荐tab 
              continue;
            } else if (/chaohua/.test(tab?.flowId)) {
              // 关注页超话tab 
              continue;
            } else {
              newTabs.push(tab);
            ...
            if (item?.items?.length > 0) {
              ...
              for (let i of item.items) {
                if (i?.data.hasOwnProperty("promotion")) {
                  // 热搜列表中的推广项目 
                  continue;
                } else if (/img_search_stick/.test(i?.data?.pic)) {
                  // 手动置顶的微博热搜 
                  continue;
                } else {
                  newII.push(i);
        ...
        // 顶部 tab：只保留「关注」页下的「微博」（最新微博）一个 tab
        if (obj?.pageDatas?.length > 0) {
          let newTabs = [];
          for (let item of obj.pageDatas) {
            // 移除「扩展页」（homeExtend）和「推荐页」（homeHot）
            if (item?.pageDataType === "homeExtend") continue;
            if (item?.pageDataType === "homeHot") continue;
            // 处理「关注页」（homeFeed）内的分组，只保留「最新微博」并改名为「微博」
            if (item?.categories?.length > 0) {
              let newCates = [];
              for (let i of item.categories) {
                if (i?.title === "默认分组" && i?.pageDatas?.length > 0) {
                  let newII = [];
        ...
        if (obj?.cards?.length > 0) {
          ...
          obj.cards = obj.cards.filter(
            (i) =>
              !(
                i?.itemid?.includes("feed-invite") || // 超话里的好友 
        ...
        if (obj?.toolbar_menus_new?.items?.length > 0) {
          ...
          for (let item of toolbar.items) {
            if (item?.identifier === "recommend") {
              // 相关推荐 
              continue;
            } else if (/reward/.test(item?.identifier)) {
              // 赞赏 
              continue;
            } else {
              newTools.push(item);
        ...
        if (obj?.items?.length > 0) {
          ...
          for (let item of obj.items) {
            ...
            if (itemId === "profileme_mine") {
              ...
              if (item?.items?.length > 0) {
                for (let d of item.items) {
                  if (d.itemId === "mainnums_friends") {
                    ...
                    d.click.modules[0].scheme = s.replace("231093-selfrecomm", "231093-selffollowed");
                  }
                }
              }
              newItems.push(item);
            } else if (itemId === "100505-top8") {
              if (item?.items?.length > 0) {
                item.items = item.items.filter(
                  (i) =>
                    i?.itemId === "100505-album" || // 我的相册 
                    i?.itemId === "100505-like" || // 赞/收藏 
                    i?.itemId === "100505-watchhistory" || // 浏览记录 
                    i?.itemId === "100505-draft" // 草稿箱 
                );
              }
              newItems.push(item);
            } else if (itemId === "100505-manage") {
              if (item?.style) {
                delete item.style;
              }
              if (item?.images) {
              ...
              newItems.push(item);
            } else if (itemId === "100505-chaohua" || itemId === "100505-recentlyuser") {
              newItems.push(item);
            } else {
              // 移除其他推广 
              continue;
        ...
        if (obj?.floating_windows?.length > 0) {
          obj.floating_windows = obj.floating_windows.filter((i) => !/(?:^ad?|red_pocket|ug_high_priority)/.test(i?.subtype));
        }
          ...
          for (let item of obj.items) {
            if (item?.type !== "trend" && item?.type !== "comment_header_tip") {
              if (item?.data) {
                ...
                if (item?.data?.comment_bubble) delete item.data.comment_bubble;
                if (item?.data?.comment_bullet_screens_message) delete item.data.comment_bullet_screens_message;
                if (item?.data?.hot_icon) delete item.data.hot_icon;
                if (item?.data?.vip_button) delete item.data.vip_button;
              }
              if (item?.items?.length > 0) {
                let newII = [];
                for (let ii of item.items) {
                  if (ii?.data) {
                    ...
                    if (ii?.data?.comment_bubble) delete ii.data.comment_bubble;
                    if (ii?.data?.comment_bullet_screens_message) delete ii.data.comment_bullet_screens_message;
                    if (ii?.data?.hot_icon) delete ii.data.hot_icon;
                    if (ii?.data?.vip_button) delete ii.data.vip_button;
                  }
                  newII.push(ii);
                }
                item.items = newII;
          ...
          for (let item of obj.items) {
            if (!isAd(item?.data) && !isAd(item?.status)) {
              if (item?.category === "dynamic") {
                if (item?.status?.action_button_icon_dic) delete item.status.action_button_icon_dic;
                ...
                if (item?.status?.user?.unfollowing_recom_switch === 1) item.status.user.unfollowing_recom_switch = 0;
                if (item?.status?.tag_struct?.length > 0) item.status.tag_struct = [];
                newItems.push(item);
              } else if (item?.category === "feed") {
                if (item?.data?.action_button_icon_dic) delete item.data.action_button_icon_dic;
                removeFeedAd(item?.data);
                removeVoteInfo(item?.data);
                ...
                if (item?.data?.user?.unfollowing_recom_switch === 1) item.data.user.unfollowing_recom_switch = 0;
                if (item?.data?.tag_struct?.length > 0) item.data.tag_struct = [];
                newItems.push(item);
        ...
        const item = [
          "bubble_guide_data", "button_extra_info", "display_info", "extend_info",
          "floating_button", "follow_data", "head_cards", "highlight",
          "interaction_extra_info", "page_alerts", "reward_info", "source_tag_struct", "top_cards"
        ];
        if (obj) {
          item.forEach((i) => {
            delete obj[i];
        ...
        if (obj?.hot_reposts?.length > 0) {
          ...
          for (let item of obj.hot_reposts) {
            if (!isAd(item)) newReposts.push(item);
          }
        ...
        removeFeedAd(obj);
        if (obj?.text) removeFeedAd(obj.text);
        if (obj?.reward_info) delete obj.reward_info;
        removeVoteInfo(obj);
        ...
        if (obj?.statuses?.length > 0) {
          ...
          for (let item of obj.statuses) {
            if (!isAd(item)) {
              removeAvatar(item);
              if (item?.video_info?.tags?.length > 0) item.video_info.tags = [];
              newStatuses.push(item);
            }
          }
          obj.statuses = newStatuses;
        ...
        if (obj?.status?.video_info?.shopping?.length > 0) obj.status.video_info.shopping = [];
        if (obj?.status?.video_info?.bottom_banner) obj.status.video_info.bottom_banner = {};
        if (obj?.status?.video_info?.float_info) obj.status.video_info.float_info = {};
      } else if (url.includes("/2/video/tiny_stream_video_list")) {
        if (obj?.statuses?.length > 0) obj.statuses = [];
        if (obj?.tab_list?.length > 0) obj.tab_list = [];
      } else if (url.includes("/2/!/huati/discovery_home_bottom_channels")) {
        if (obj?.button_configs) delete obj.button_configs;
        ...
        if (obj?.data?.list?.length > 0) {
          for (let item of obj.data.list) {
            if (item?.cardType) item.cardType = 2;
          }
        }
        ...
        if (obj?.cached_ad?.ads?.length > 0) {
          for (let item of obj.cached_ad.ads) {
            item.show_count = 50;
            item.duration = 0;
            item.start_date = 3818332800;
            item.end_date = 3818419199;
          }
        }
      }
    ...
    // 捕获异常，遇到非规范返回时直接静默放行原始数据
    console.log("Weibo Script JSON Parse Error: " + error);
    $done({});
  }
}
...
function isAd(data) {
  if (data?.mblogtypename === "广告") return true;
  if (data?.mblogtypename === "热推") return true;
  if (data?.readtimetype === "adMblog") return true;
  if (data?.promotion?.recommend === "广告") return true;
  if (data?.promotion?.recommend === "热推") return true;
  if (data?.promotion?.type?.includes("ad")) return true;
...
function removeAvatar(data) {
  if (data?.block_card_bg) delete data.block_card_bg;
  if (data?.buttons) delete data.buttons;
  if (data?.cardid) delete data.cardid;
  if (data?.icons) delete data.icons;
  if (data?.mblog_buttons) delete data.mblog_buttons;
  if (data?.pic_bg_new) delete data.pic_bg_new;
  if (data?.user?.avatargj_id) delete data.user.avatargj_id;
  if (data?.user?.avatar_extend_info) delete data.user.avatar_extend_info;
  if (data?.user?.cardid) delete data.user.cardid;
  if (data?.user?.icons) delete data.user.icons;
  if (data?.user?.mbtype) delete data.user.mbtype;
}

function removeFeedAd(item) {
  removeAvatar(item);
  if (item?.retweeted_status) removeAvatar(item?.retweeted_status);
  if (item?.action_button_icon_dic) delete item.action_button_icon_dic;
  if (item?.common_struct) delete item.common_struct;
  if (item?.comment_summary) delete item.comment_summary;
  if (item?.extend_info) delete item.extend_info;
  if (item?.icons_material) delete item.icons_material;
  if (item?.semantic_brand_params) delete item.semantic_brand_params;
}
