...
class UserInfo {
    ...
    async getTaskList() {
        try {
            const opts = {
                url: "https://xxyx-client-api.xiaoxiaoyouxuan.com/client/energy/mall/getTaskList",
            }
            let res = await this.fetch(opts);
    ...
    async checkLogin() {
        try {
            const opts = {
                url: `/client/user/bind/leader`,
                type: "post",
                dataType: "json",
                body: { "signalId": 202, "uid": "XQIGUQVUAwxZClwM" }
            }
            return await this.fetch(opts);
        } catch (e) {
            this.ckStatus = false;
...
async function getCookie() {
    try {
        ...
        if (!(header["xx-token"] && body?.data?.mobile)) throw new Error("获取token失败！参数缺失");

        const newData = {
            "userId": body?.data?.identityId,
            "token": header["xx-token"],
            "userName": body?.data?.mobile
        }
