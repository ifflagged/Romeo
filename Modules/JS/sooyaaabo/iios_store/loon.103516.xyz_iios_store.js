/**
 * @name iios.fun 自动签到
 * @description 支持 Argument 传入邮箱、密码、client_id
...
function saveArgumentToStore() {
  ...
  if (args.email !== '') Env.write(EMAIL_KEY, args.email);
  if (args.password !== '') Env.write(PASSWORD_KEY, args.password);
  if (args.clientId !== '') Env.write(CLIENT_ID_KEY, args.clientId);
  return args;
}
...
if (isRequestPhase()) {
  ...
  async function main() {
    ...
    Env.notify('iios.fun', '开始登录签到', `账号：${email}`);
  ...
  .finally(() => {
    ...
    Env.notify(finalResult.title, finalResult.subTitle, finalResult.message);
    Env.done({ ok: 1 });
  });
}