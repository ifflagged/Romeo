...
if (requestUrl.includes(path1)) {
  ...
  (async () => {
    ...
    try {
      ...
      const errorHtml = `<div style="padding: 20px; text-align: center; color: red; font-size: 16px;"><strong>比价失败：</strong><span onclick="${clickAction}" style="cursor: pointer; text-decoration: underline;">${
        err.message || "未知错误"
      }</span></div>`;
...
function jsonToCustomString(jsonObject) {
  return Object.keys(jsonObject)
    .filter((key) => jsonObject[key] != null && jsonObject[key] !== "" && key.toLowerCase() !== "token")
    .sort()
